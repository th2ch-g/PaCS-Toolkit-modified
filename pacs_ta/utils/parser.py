...
from pacs_ta.utils.fit import fit, fit_trial
from pacs_ta.utils.gencom.gencom_traj import gencom_traj
from pacs_ta.utils.gencom.gencom_trial import gencom_trial
from pacs_ta.utils.genfeature import comdist, comvec, rmsd, xyz
from pacs_ta.utils.genrepresent import genrepresent
from pacs_ta.utils.logger import generate_logger
from pacs_ta.utils.rmfile import rmfile_all
from pacs_ta.utils.rmmol import make_top, rmmol_all, rmmol_log_add_info
...
class Parser:
    def parse(self) -> MDsettings:
        ...
        parser_mdrun.add_argument(
            ...
            type=str,
            required=True,
            help="trial number without 0-fill when pacs_tamd was conducted (e.g. -t 1)",
        )
        parser_mdrun.add_argument(
            "-f",
            "--file",
        ...
        parser_fit_trial = parser_fit_sub.add_parser(
            "trial",
            help="apply to single pacs_tamd trial",
        )
        ...
        parser_gencom_trial = parser_gencom_sub.add_parser(
            "trial", help="apply single pacs_tamd trial"
        )
        ...
        # pacs_tamd -> error
        if len(sys.argv) == 1:
            LOGGER.error('Use "pacs_ta -h" to view help messages')
            exit(1)
    ...
    def check_version(self, trial_dir: str):
        version_file = f"{trial_dir}/pacs_tatk.version"
